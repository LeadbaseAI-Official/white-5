...
def get_pipeline() -> AutoPipelineForText2Image:
    ...
    if _pipeline is None:
        ...
        print(f"[Model] Loading model ID '{MODEL_ID}' on device '{device}' with dtype '{dtype}'...", flush=True)
        ...
        _pipeline.vae = vae
        _pipeline.to(device)
        print("[Model] Model loaded successfully with SD-Turbo and Tiny VAE.", flush=True)
    return _pipeline
...
def generate_image(prompt: str, num_inference_steps: int = 1, guidance_scale: float = 0.0, width: int = 512, height: int = 512) -> Image.Image:
    ...
    print(f"[Model] Starting image generation for prompt: '{prompt}' (steps={num_inference_steps}, guidance={guidance_scale}, size={width}x{height})...", flush=True)
